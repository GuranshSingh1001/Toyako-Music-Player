...
enum LibraryCategory: Hashable {
    case songs, albums, artists, allPlaylists
    case playlist(UUID)
}
...
struct ContentView: View {
    ...
    private func tabContent(for category: LibraryCategory) -> some View {
        NavigationStack {
            detailContent(for: category)
                ...
                // Injecting MiniPlayer here prevents it from overlapping the sidebar
                .safeAreaInset(edge: .bottom) {
                    if audioManager.currentTrack != nil {
                        MiniPlayerView()
                            .glassEffect(.regular.interactive(), in: .capsule) // Perfectly rounded Apple Music pill
                            .shadow(color: .black.opacity(0.2), radius: 15, y: 8)
                            .contentShape(Rectangle())
                            .onTapGesture {
                                withAnimation(.interpolatingSpring(stiffness: 250, damping: 25)) { showNowPlaying = true }
                            }
    ...
    private func detailContent(for category: LibraryCategory) -> some View {
        ...
        case .playlist(let id):
            if let pl = library.playlists.first(where: { $0.id == id }) {
                ...
                SongListView(
                    tracks: filterTracks(pTracks), allTracks: pTracks, library: library, playlistID: pl.id,
                    headerView: searchText.isEmpty ? AnyView(PlaylistHeaderView(playlist: pl, tracks: pTracks, onAddSongs: { playlistToEdit = pl })) : nil
                )
            }
        }
    }
    ...
    private func filterTracks(_ source: [LocalTrack]) -> [LocalTrack] {
        ...
        return source.filter { $0.title.localizedCaseInsensitiveContains(searchText) || $0.artist.localizedCaseInsensitiveContains(searchText) }
    }

    private var filteredTracks: [LocalTrack] { filterTracks(library.tracks) }
    private var filteredAlbums: [AlbumGroup] { searchText.isEmpty ? library.albums : library.albums.filter { $0.name.localizedCaseInsensitiveContains(searchText) } }
    private var filteredArtists: [ArtistGroup] { searchText.isEmpty ? library.artists : library.artists.filter { $0.name.localizedCaseInsensitiveContains(searchText) } }
}